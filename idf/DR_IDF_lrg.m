function [data]=DR_IDF_lrg(data,Week,T_low,T_high,startT_init,endT_init,timestep)
if timestep ==1
startT=strcat(num2str(floor(startT_init)),':00');
endT=strcat(num2str(floor(endT_init)),':00');
else
    startT=strcat(num2str(floor(startT_init)),':',num2str(floor((startT_init-floor(startT_init))*timestep)),'0');
    endT=strcat(num2str(floor(endT_init)),':',num2str(floor((endT_init-floor(endT_init))*timestep)),'0');
end
% DR=struct;
default_weekstart={'OfficeLarge CLGSETP_SCH_NO_SETBACK Default'};
default_weekend={'OfficeLarge CLGSETP_SCH_NO_SETBACK Default|WntrDsn|Sat|Wkdy Day', 'OfficeLarge CLGSETP_SCH_NO_SETBACK Default',...
    'OfficeLarge CLGSETP_SCH_NO_SETBACK Summer Design Day',...
    'OfficeLarge CLGSETP_SCH_NO_SETBACK Winter Design Day','OfficeLarge CLGSETP_SCH_NO_SETBACK Default',...
    'OfficeLarge CLGSETP_SCH_NO_SETBACK Default'};

% skip=1;
k=1;
flag_end=0; %flag end =1 if the new schedule starts in first week  of the year.,
date_det={'OfficeLarge CLGSETP_SCH_NO_SETBACK','Temperature'};
Def_Tsetpoint='29.44';
Tsetpoint='23.89';

for i=1:length(Week)
    m=0;
    check=0;
    week_det={sprintf('Schedule:Week:Daily {Choghondar%d}',i)};% the problem is when we have a week with less days . need fixing.
    week_det=[week_det,default_weekstart];
    for j=1:length(Week(i).day)
        if ~(j+1==Week(i).day(j).Order)&m==0
            m=m+1;
            check=1;
            j=j-1;
        elseif j+m+1==Week(i).day(j).Order & m~=0
            check=0;
            m=m+1;
        end
        if ~strcmpi(Week(i).day(j).start,Week(i).day(j).end)& ~check
            sch_name=sprintf('OfficeLarge CLGSETP_SCH_NO_SETBACK Default|WntrDsn|Sat|Wkdy Day %d',k);
            sch_det={sch_name,'Temperature',' No','06:00',Def_Tsetpoint,startT,...
                num2str(Tsetpoint),endT,num2str(T_low),...
                '18:00',num2str(T_high),'24:00',Def_Tsetpoint};
            sch.class='Schedule:Day:Interval';
            sch.fields=sch_det;
            if j==1
                sch_chunk=sch;
            else
                sch_chunk=[sch_chunk;sch];
            end
            
            
            
        end
        if check
            sch_name={sprintf('OfficeLarge CLGSETP_SCH_NO_SETBACK Default|WntrDsn|Sat|Wkdy Day')};
        end
        week_det=[week_det,sch_name];
        k=k+1;
    end
    week_det=[week_det,default_weekend];
    week.class='Schedule:Week:Daily';
    week.fields=week_det;
    if i==1
        week_chunk=week;
    else
        week_chunk=[week_chunk;week];
    end
    if (Week(i).DayStart>=2) & (Week(i).MonthEnd>=1)&i==1
        date_det=[date_det,'Schedule:Week:Daily {06cd666e-1c2d-4e1f-b109-113b36e9206b}'];
        date_det=[date_det,'1','1',num2str(Week(i).MonthStart),num2str(Week(i).DayStart-1)];
    end
    
    if (Week(i).DayEnd>=26) & (Week(i).MonthEnd==12)
        date_det=[date_det,sprintf('Schedule:Week:Daily {Choghondar%d}',i)];
        date_det=[date_det,num2str(Week(i).MonthStart),num2str(Week(i).DayStart),'12','31'];
        flag_end=1;
        
    else
        date_det=[date_det,sprintf('Schedule:Week:Daily {Choghondar%d}',i)];
        date_det=[date_det,num2str(Week(i).MonthStart),num2str(Week(i).DayStart),num2str(Week(i).MonthEnd),num2str(Week(i).DayEnd)];
    end
    
end
if flag_end~=1
    date_det=[date_det,'Schedule:Week:Daily {06cd666e-1c2d-4e1f-b109-113b36e9206b}'];
    date_det=[date_det,num2str(Week(i).MonthStart),num2str(Week(i).DayEnd+1),'12','31'];
    
end
date_chunk.class='Schedule:Year';
date_chunk.fields=date_det;

for i=1:length(data)
    if strfind(data(i).class,'Schedule:Year')
        if (cellfun(@any,(strfind((data(i).fields(1)),'OfficeLarge CLGSETP_SCH_NO_SETBACK'))))
            aa=i;
            break
        end
    end
end

data(aa).fields=date_chunk.fields;
for i=1:length(sch_chunk)
    data=[data,sch_chunk(i)];
end
for i=1:length(week_chunk)
    data=[data,week_chunk(i)];
end

end